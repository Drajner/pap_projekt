...
public class CLI { // testing
    ...
    public String ANSI_RGB(ArrayList<Integer> color, String text) {
        return "\u001B[38;2;" + color.get(0) + ";" + color.get(1) + ";" + color.get(2) + "m" + text + ANSI_RESET;
    }

    public void interline(int length, int R, int G, int B) {
        ArrayList<Integer> color = new ArrayList<Integer>();
        color.add(R);
        color.add(G);
        color.add(B);
        for (int i = 0; i < length; i++) {
            System.out.print(ANSI_RGB(color, "=") + ANSI_RESET);
        }
    }

    public void interlineGradient(int length, int R1, int G1, int B1, int R2, int G2, int B2) {
        ArrayList<Integer> color = new ArrayList<Integer>();
        color.add(R1);
        color.add(G1);
        color.add(B1);
